...
public color col = color(255,0,0,50);
  ...
  for(int i = 0; i < fft.specSize()-1; i++)
  {
   ...
   fftL.logAverages(11, 16);
    ...
    if (buttonPlay){
      ...
      int sizeFft = int (1+5*fft.getBand(i));
      if (sizeFft >100){
        sizeFft = 100;
      }
      if (sizeFft <0){
    ...
    //  println(sizeFft);
      ...
      ellipse(i , 250 , valueSlider2/14.2857+0.01,fft.getBand(i)%valueSlider1 + 15);
    }else if (!buttonPlay){
      rect(1+5*0.01,2,2,20);
      ellipse(i , 250 , valueSlider2/14.2857+0.01,fft.getBand(i)%0.01 + 15);
    }
 ...
 for (int i=1; i <= fftR.specSize(); i+=fftR.specSize()/grid) { //for (int i=1; i <= fftL.specSize(); i+=fftL.specSize()/grid) {
     if (buttonPlay){
      ...
      if (second()%0.5==0){
        ...
        if (MyController1.getController("Beat pulse / Volume").getValue()<0.2){
       ...
       MyController1.getController("Beat pulse / Volume").setValue(0);
      x = map(i, 0, (fftR.specSize()+fftL.specSize())/2, 0, width);
      y = map((fftR.getBand(i)+fftL.getBand(i))/2*0.01*yScale%valueSlider1, 0, 100, 0, height/3) ;
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
       fill (102, 100, 250, 85);
...
public void rgbToHsv(Color colorDisplay){
  ...
  println(int(hsv[0]*360)+"/"+int(hsv[1]*100)+"/"+int(hsv[2]*100)+"\n");
