...
public class beatDetection extends PApplet {
...
public void setup()
{
  ...
  displayColors = new int[numBands];
...
int pixelOnColor = color(25, 25, 25, 255);
int pixelOffColor = color(0, 0, 0, 255);

boolean pixelsOn = true;
public void togglePixels(int d)
{
  int pixelsColor;
  if (pixelsOn)
  {
...
public void detectBeat()
{
  ...
  float w = (size * 1.0f) / (numBands * 1.0f);
  float level = abs(in.mix.level()) * levelScale * 100 * 2;

  for (int bandIndex = 0; bandIndex < numBands; bandIndex++)
  {
    int band = (int)(w * (bandIndex + 1) - w / 2.0f);
    ...
    eRadius[bandIndex] *= 0.8f;
  }
}
public void drawCircle(int bandIndex, int numBands, int band, float level, float radius)
{
  ...
  public void settings() {  size(1000, 400, P3D); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "beatDetection" };
    if (passedArgs != null) {
